var Fisarmonica = function (options) {
    // eslint-disable-line no-unused-vars, padded-blocks
    ...
    var _setStatus = function (arrayOfItems, action) {
        panels.forEach(function (panel, idc) {
            if (action === 'open') {
                if (arrayOfItems) {
        ...
        buttons.forEach(function (button, idb) {
            if (action === 'open') {
                if (arrayOfItems) {
                    if (arrayOfItems.includes(idb)) {
                        button.setAttribute('aria-expanded', true);
                        if (arrayOfItems.length === 1) {
                            // focus only if we have to open 1 panel
                            button.focus();
                        }
                    }
                } else {
    ...
    var _changeStyles = function () {
        if (mergedOptions.theme) {
            for (var style in mergedOptions.theme) {
                accordionRootElement.style.setProperty(
                    '--' + style,
                    mergedOptions.theme[style],
                );
            }
        }
    };
    ...
    var _prepareMarkup = function () {
        ...
        buttons = Array.from(
            accordionRootElement.querySelectorAll('dt > button'),
        );
        ...
        buttons.forEach(function (button, idb) {
            ...
            button.setAttribute(
                'aria-controls',
                classes.cssMainClass + '-' + accordionId + '-' + idb,
            );
            button.setAttribute(
                'id',
                classes.cssMainClass + '-' + accordionId2 + '-' + idb,
            );
        });
        panels.forEach(function (panel, idc) {
            ...
            panel.setAttribute(
                'id',
                classes.cssMainClass + '-' + accordionId + '-' + idc,
            );
            panel.setAttribute(
                'aria-labelledby',
                classes.cssMainClass + '-' + accordionId2 + '-' + idc,
            );
        });
    };
    ...
    var _setFocusWatcher = function () {
        buttons.forEach(function (button) {
            ...
            button.addEventListener('blur', function () {
                accordionRootElement.classList.remove(
                    classes.accordionFocusClass,
                );
            });
        });
    };
    ...
    var _keyboardEvents = function () {
        accordionRootElement.addEventListener('keydown', function (e) {
            ...
            if (index !== -1) {
                // do all the logic only if we are acting on a trigger element
                if (
                    key === '40' ||
                    key === '38' ||
                    key === '36' ||
                    ...
                    if (key === '40' || key === '38') {
                        ...
                        index =
                            key === '40'
                                ...
                                : index - 1 === -1
                                ? buttons.length - 1
                                : index - 1; // check if we are at the end or at the beginning
                    } else if (key === '36') {
                        // home
                        index = 0;
                    } else if (key === '35') {
    ...
    var _init = function () {
        if (!accordionRootElement) {
            console.warn(
                'Cannot find an fisarmonica with class' +
        ...
        _keyboardEvents();
        fisarmonica.close();
        accordionRootElement.addEventListener(
            'click',
            _rootClickHandler,
            false,
        );
